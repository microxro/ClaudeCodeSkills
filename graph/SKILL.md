---
name: graph
description: Build and query a persistent, confidence-tagged knowledge graph of a repository (files, classes, functions, imports, calls, inheritance, docs) instead of rediscovering its structure from scratch every time you need codebase context. Use this whenever a task needs you to gather context about how a codebase is put together to do the actual work well — a code review, "what does this PR touch and what depends on it", architecture questions ("how does auth connect to the database", "what depends on this service", "what's the path from the API layer to the DB", "what would break if I changed this"), prep before a large refactor, or general unfamiliar-codebase exploration — or whenever the user asks to "graph", "map", or "graphify" the repo. If a graph already exists for the repo (check for `.graph/graph.json`), query it first and read only the exact source files it points to, instead of grepping/reading broadly — that's the whole point: it saves tokens versus rediscovering structure from scratch on every task that touches more than one file. Skip this for a small repo, a single-file task, or any question about what a specific line of code actually does (that's a job for reading the file directly, not the graph).
---
