---
name: taste
description: Apply practitioner judgment instead of generic-model defaults when creating or critiquing any real deliverable — code, UI, documents, plans, messages, system designs, charts. Ground in the actual job, the audience, and a real inspected exemplar; choose the right shape for the content instead of a default template; rank material by how much it actually matters instead of giving everything equal weight; use only source-backed specifics instead of inventing plausible detail; commit to one recommendation instead of a menu of options; then subtract and quality-gate before calling it done. For reviewing existing work, give a short verdict plus the few things that actually matter, not an exhaustive rubric. Use this whenever the user says "use taste" / "apply taste" / "make this good" / "tighten this" / "polish this" / "make this less generic", for any deliverable someone else will see or that has real stakes (a proposal, a UI, a PR, a landing page, a report, a message to a client), whenever a brief is thin and there's pressure to invent specifics to look complete, or for any review/critique/comparison request — even if the user never says the word "taste". Do NOT use it for verbatim transformation, exhaustive extraction, or any task where completeness/coverage IS the explicit goal (e.g. "list every function name in these files") — taste is not permission to editorialize a coverage task.
---
